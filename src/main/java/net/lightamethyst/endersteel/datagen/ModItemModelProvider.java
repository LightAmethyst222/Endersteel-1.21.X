...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    protected void registerModels() {
        ...
        basicItem(ModItems.ENDERSTEEL_UPGRADE_SMITHING_TEMPLATE.get());

        basicItem(ModItems.STEEL_INGOT.get());
        basicItem(ModItems.RAW_STEEL.get());
        basicItem(ModItems.STEEL_UPGRADE_SMITHING_TEMPLATE.get());
        ...
        saplingItem(ModBlocks.ENDERWOOD_SAPLING);
    }

    // Trimable Armor Helper Method
    // Shoutout to El_Redstoniano for making this
    private void trimmedArmorItem(DeferredItem<ArmorItem> itemDeferredItem) {
        final String MOD_ID = Endersteel.MOD_ID;
    ...
    public ItemModelBuilder handheldItem(DeferredItem<?> item) {
        return withExistingParent(item.getId().getPath(),
                ...
                ResourceLocation.fromNamespaceAndPath(Endersteel.MOD_ID, "item/" + item.getId().getPath()));
    }
    private ItemModelBuilder saplingItem(DeferredBlock<Block> item) {
        return withExistingParent(item.getId().getPath(),
                ResourceLocation.parse("item/generated")).texture("layer0",
                ResourceLocation.fromNamespaceAndPath(Endersteel.MOD_ID,"block/" + item.getId().getPath()));
    }
}
